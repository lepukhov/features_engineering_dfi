...
calculate_psi <- function(df, target_col, date_col, n_intervals = 6, min_bucket_size = 50) {
  ...
  if (n_intervals > 0) {
    for (i in seq_len(n_intervals)) results[[paste0("PSI_Interval_", i)]] <- NA_real_
  }
  ...
  for (var_idx in seq_along(feature_cols)) {
    ...
    if (n_intervals >= 2) {
      baseline <- woe_by_interval[[1]]
      for (i in 2:n_intervals) {
        if (!is.na(baseline) && !is.na(woe_by_interval[[i]])) {
          dist1 <- c(baseline, 1 - abs(baseline))
